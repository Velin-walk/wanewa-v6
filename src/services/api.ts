...
export function normalizeTrek(row: any): Trek {
  const difficulty = String(row.difficulty || "Easy").toLowerCase();
  return {
    id: String(row.id || row.hike_number || row.trek_id || row.trek_name || row.name),
    hike_number: String(row.hike_number || ""),
    name: row.name || row.trek_name || "",
    date: row.date || "",
    days: row.days || "1",
    difficulty: difficulty === "hard" ? "difficult" : difficulty === "moderate" ? "moderate" : "easy",
    leader: row.leader || row.team_leader || "Walk Nepal Walk Guide",
    capacity: Number(row.capacity || row.max_capacity) || 25,
    ...
    price: row.price || "",
    featured_image: row.featured_image || row.thumbnail_url || "",
    fitness_level: row.fitness_level || "All fitness levels",
